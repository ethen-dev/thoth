...
skills.command("run").argument("<id>", "Skill id").requiredOption("--input <json>", "JSON input").option("--mode <mode>", "validate, plan, dry-run or execute", "execute").option("--confirmed", "Confirm the proposed mutation").option("--token <token>", "Exact confirmation token from plan/dry-run").description("Run a skill without executing shell, Markdown, or implicit providers").action(async (id: string, options: { input: string; mode: "validate" | "plan" | "dry-run" | "execute"; confirmed?: boolean; token?: string }) => {
  try { let input: unknown; try { input = JSON.parse(options.input); } catch { throw new Error("--input must be valid JSON"); } const result = await runSkill(await loadConfig(), { skillId: id, input, mode: options.mode, confirmed: options.confirmed, confirmationToken: options.token }); console.log(JSON.stringify(result, null, 2)); if (!result.ok) process.exitCode = 1; } catch (error) { reportError(error); }
});
